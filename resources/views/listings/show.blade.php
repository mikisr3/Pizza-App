
<x-layout>

@include('partials._search')

 <a href="/" class="inline-block text-black ml-4 mb-4"
                ><i class="fa-solid fa-arrow-left"></i> Back
            </a>
            <div class="mx-4">
                <x-card>
                    <div
                        class="flex flex-col items-center justify-center text-center"
                    >
                        <img
                            class="w-48 mr-6 mb-6"
                             src="{{$listing->logo ? asset('storage/'.$listing->logo) : asset('/images/pizza-logo.png')}}"
                            alt=""
                        />

                        <h3 class="text-2xl mb-2">{{$listing->name}}</h3>
                        <div class="text-xl font-bold mb-4 pt-2 text-red-700">{{$listing->company}}</div>

                        <x-listing-tags :tagsCsv="$listing->tags"/>

                        <div class="text-lg my-4">
                            <i class="fa-solid fa-location-dot pr-4"></i>{{$listing->location}}
                        </div>
                        <div class="border border-gray-200 w-full mb-6"></div>

                        @if($listing->coordinates)

                       @php 

                       $tags = explode(',',$listing->coordinates); @endphp



                            <div class="container mt-5">

                                   <h3 class="text-3xl font-bold mb-4">
                                         Shop Location
                                   </h3>
                                    <div style="height: 400px;" id="map"></div>

                            </div>
                                          
                                            <script type="text/javascript">

                                                var latitude = {!! json_encode($tags[0]) !!};

                                                var longitude = {!! json_encode($tags[1]) !!};


                                                function initMap() {

                                                  const myLatLng = { lat: parseFloat(latitude), lng: parseFloat(longitude) };
                                                  const map = new google.maps.Map(document.getElementById("map"), {
                                                    zoom: 15,
                                                    center: myLatLng,
                                                  });
                                          
                                                  new google.maps.Marker({
                                                    position: myLatLng,
                                                    map,
                                                    title: "",
                                                  });
                                                }
                                          
                                                window.initMap = initMap;
                                            </script>
                                          
                                            <script type="text/javascript"

                                                src="https://maps.google.com/maps/api/js?key={{ env('GOOGLE_MAP_KEY') }}&callback=initMap" >
                                                    

                                            </script>
                        <div>

                         <div class="border border-gray-200 w-full mb-6"></div>

                         @endif

                            <h3 class="text-3xl font-bold mb-4">
                                Pizza Description
                            </h3>
                            <div class="text-lg space-y-6">

                              

                               {!! nl2br(@$listing->description) !!}

                              {{--  {{str_replace(array("\r\n","\r","\n","\\r","\\n","\\r\\n"),'&nbsp',$listing->description)}} --}}

                                <a
                                    href="mailto:{{$listing->email}}"
                                    class="block bg-laravel text-white mt-6 py-2 rounded-xl hover:opacity-80"
                                    ><i class="fa-solid fa-envelope"></i>
                                    Contact Shop</a
                                >

                                <a
                                    href="//{{$listing->website}}"
                                    target="_blank"
                                    class="block bg-black text-white py-2 rounded-xl hover:opacity-80"
                                    ><i class="fa-solid fa-globe"></i> Visit
                                    Website</a
                                >
                            </div>
                        </div>
                    </div>
                </x-card>

                {{-- <x-card class="mt-4 p-2 flex space-x-6">

                    <a href="/listings/{{$listing->id}}/edit">
                        <i class="fa-solid fa-pencil"></i>Edit
                    </a>

                    <form method="POST" action="/listings/{{$listing->id}}">

                        @csrf

                        @method('DELETE')

                        <button class="text-red-500"><i class="fa-solid fa-trash"></i></button>
                        




                    </form>
                    

                </x-card> --}}
            </div>

            </x-layout>

    

    